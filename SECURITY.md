...
| A04 | Cryptographic Failures | ⚠️ Partly | bcrypt password hashing; `SECRET_KEY` never ships as a static default (auto-generated and persisted, historical placeholder explicitly rejected); staff tokens no longer travel in URLs. **Open:** no TLS by default, so credentials and tokens cross the network in the clear; and the desktop client's `client_id` -- which the public chat endpoints treat as a bearer secret -- is stored as plaintext `client_id.txt` in the user's profile. |
...
- **Client secret stored in plaintext.** The desktop client's `client_id` grants read access to that user's chat history and lives in a readable file in the user's profile. Moving it to Windows Credential Manager / macOS Keychain needs the `keyring` package, which is a dependency addition and is deliberately not taken.
